...
def fetch_data_from_url(job_url):
    loader = WebBaseLoader(job_url)
    page_data = loader.load().pop().page_content
    #print(page_data)
    return page_data

def access_perticular_data(llm, page_data, data_extract):
    prompt_extract = PromptTemplate.from_template(data_extract)
...
def create_email(llm, job, links, output_email):
    prompt_email = PromptTemplate.from_template(output_email)
...
def main(groq_api, my_portfolio, job_url, data_extract, output_email):
    # create llm model
    ...
    #extract job posting data from html content
    data = access_perticular_data(llm_model, url_all_content, data_extract)
    #print(data)
    ...
    email = create_email(llm_model, job, links, output_email)
    print(email)
...
if __name__ == "__main__":
    ...
    groq_api     = "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"      
    ...
    main(groq_api, my_portfolio, job_url, data_extract, output_email)
